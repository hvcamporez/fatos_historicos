...
function mostrarPergunta() {
  const perguntaAtual = questoesSelecionadas[indiceAtual];

  document.getElementById("contador").textContent =
    `Pergunta ${indiceAtual + 1} de ${questoesSelecionadas.length}`;
...
function enviarResposta() {
  ...
  if (respostaUsuario === perguntaAtual.resposta) {
    ...
    document.getElementById("feedback").textContent =
      `❌ Resposta errada! Correto: ${perguntaAtual.resposta}`;
  ...
  setTimeout(() => {
    ...
    if (indiceAtual < questoesSelecionadas.length) {
      mostrarPergunta();
    } else {
      mostrarResultado();
    }
...
function mostrarResultado() {
  ...
  document.getElementById("resultadoFinal").textContent =
    `${nomeJogador}, você fez ${pontuacao} de 10 pontos.\n${mensagem}`;
}
